...
class ClientSessionHandler {
	...
	if (sessionRefId != null) {
		...
		public void run() {
			...
			dataService, graceful, true);
                    if (sessionService.disconnectSessionOp != null) {
                        sessionService.disconnectSessionOp.report();
                    }
		}
	    });
	}
	...
	/** Processes the received message. */
		...
		taskQueue.addTask(new AbstractKernelRunnable() {
			...
			if (sessionImpl != null) {
			    if (isConnected()) {
                                if (sessionService.receiveMessageOp != null) {
                                    sessionService.receiveMessageOp.report();
                                }
	 ...
	 */
	public void run() {
            if (sessionService.loginOp != null) {
                sessionService.loginOp.report();
            }
