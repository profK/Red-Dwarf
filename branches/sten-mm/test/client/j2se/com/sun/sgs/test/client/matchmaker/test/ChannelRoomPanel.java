...
/**
 * Common UI superclass for displaying the details of a channel of 
 * communication.
 */
public class ChannelRoomPanel extends JPanel {
    ...
    public void playerEntered(byte[] id, String userName) {
        userMap.put(byteArrayToString(id), userName);
        numUsers++;
        updateNumUsers();
    ...
    protected String removePlayer(byte[] id) {
        numUsers--;
        updateNumUsers();
        String userName = userMap.remove(byteArrayToString(id));
        parentUI.receiveServerMessage(getPrefix() + userName + " Left " +
                type + " " + name.getText());
        return userName;
    }
    
    protected void playerBootedFromGame(String booter, String bootee, 
			boolean isBanned) {

    	parentUI.receiveServerMessage(getPrefix() + booter + " has booted "  +
                            (isBanned ? "(and banned) " : "") + bootee + 
                            " from the game");
