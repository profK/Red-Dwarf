...
/**
 * An implementation of IGameChannel that uses SGS ClientChannels 
 * for its communication.
 * 
 */
public class GameChannelImpl extends AbstractChannelRoom implements GameChannel, 
                                                        ClientChannelListener {

    private GameChannelListener listener;

    public GameChannelImpl(ClientChannel chan, MatchMakingClientImpl client) {
    	super(chan, client);
    }

    public void setListener(GameChannelListener listener) {
    ...
    public void boot(String player, boolean isBanned) {
    ...
    public void receivedMessage(ClientChannel channel, SessionId sender, 
        ...
        if (command == PLAYER_ENTERED_GAME) {
            ...
            GameDescriptor game = new GameDescriptor(gameName, gameDescription, 
                            null, 0, false, gameParams);
            int errorCode = protocol.readUnsignedByte(data);
            listener.updateGameFailed(game, errorCode);
        }  
        else if (command == PLAYER_LEFT_GAME) {
            byte[] id = protocol.readBytes(data);
