 ...
 *
 * <dt> <i>Property:</i> <code><b>{@value #PROFILE_LISTENERS_PROPERTY}
 *	</b></code> <br>
 ...
 *
 * <dt> <i>Property:</i> <code><b>{@value #SHUTDOWN_TIMEOUT_PROPERTY}
 *	</b></code> <br>
 *	<i>Default:</i> <code>{@value #DEFAULT_SHUTDOWN_TIMEOUT}</code>
 *                      (2 minutes)
 *
 * <dd style="padding-top: .5em">The time in milliseconds that the node
 *      will wait for shutdown to complete. If shutdown does not complete
 *      in the specified time, the node will be forcibly shutdown by calling
 *      <code>System.exit(1)</code>. The value must be between 5000 (5 seconds)
 *      and {@link Integer#MAX_VALUE}<p>
 *
 * <dt> <i>Property:</i> <code><b>{@value #EXIT_ON_SHUTDOWN_PROPERTY}
 *	</b></code> <br>
 *	<i>Default:</i> <code>true</code>
 *
 * <dd style="padding-top: .5em">By default, when the node has completed
 *      shutdown, <code>System.exit(0)</code> is called. This can be disabled
 *      by specifying this property with the value of <code>false</code>.<p>
 *
...
class Kernel {
    ...
    private static final String PKG_NAME = "com.sun.sgs.impl.kernel";

    // the property for setting profiling levels
    public static final String PROFILE_LEVEL_PROPERTY =
        PKG_NAME + ".profile.level";

    // the property for setting the profile listeners
    public static final String PROFILE_LISTENERS_PROPERTY =
        PKG_NAME + ".profile.listeners";

    // The property for specifying the shutdown timeout
    public static final String SHUTDOWN_TIMEOUT_PROPERTY =
        PKG_NAME + ".shutdown.timeout";

    // The default timeout the kernel's shutdown method (2 minutes)
    public static final int DEFAULT_SHUTDOWN_TIMEOUT = 2 * 60000;

    // The property to enable/disabling exit(0) on shutdown
    public static final String EXIT_ON_SHUTDOWN_PROPERTY =
        PKG_NAME + ".exit.on.shutdown";

    // The property for specifying the access coordinator
    public static final String ACCESS_COORDINATOR_PROPERTY =
	PKG_NAME + ".access.coordinator";
    ...
    // specifies whether to call System.exit() on shutdown
        ...
        throws Exception 
    {
        ...
        try {
            ...
            if (logger.isLoggable(Level.SEVERE)) {
                logger.log(Level.SEVERE, "Unknown profile level {0}",
                           level);
            }
            throw iae;
        }
        shutdownTimeout =
                wrappedProperties.getIntProperty(SHUTDOWN_TIMEOUT_PROPERTY,
                                                 DEFAULT_SHUTDOWN_TIMEOUT,
                                                 5 * 1000, Integer.MAX_VALUE);
        exitOnShutdown =
                 wrappedProperties.getBooleanProperty(EXIT_ON_SHUTDOWN_PROPERTY,
                                                      true);
        try {
	    ...
	    systemRegistry.addComponent(collectionsFactory);
            ...
            logger.log(Level.CONFIG,
                       "Created Kernel with properties:" +
                       "\n  " + ACCESS_COORDINATOR_PROPERTY + "=" +
                       accessCoordinator.getClass().getName() +
                       "\n  " + EXIT_ON_SHUTDOWN_PROPERTY + "=" +
                       exitOnShutdown +
                       "\n  " + PROFILE_LEVEL_PROPERTY + "=" + level +
    ...
    private void loadProfileListeners(ProfileCollector profileCollector) {
        List<String> listenerList =
                ...
                PROFILE_LISTENERS_PROPERTY, String.class, "");
    ...
    synchronized void shutdown() {
        ...
        startShutdownTimeout(shutdownTimeout);
