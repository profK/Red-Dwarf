...
public class TestScalableHashMapStress extends Assert {
	...
	txn = createTransaction();
	int currentObjectCount = getObjectCount();
	if (debug && initialObjectCount != currentObjectCount) {
	    printObjects(null);
	}
	assertEquals(initialObjectCount, currentObjectCount);
	txn.abort(null);
    }
	...
	while (true) {
	    ...
	    last = next;
	    /*
	     * Ignore PendingTask objects, since they are not garbage.  We may
	     * need to update this check if other services create objects that
	     * hang around.  -tjb@sun.com (12/05/2007)
	     */
	    ManagedObject object = dataService.createReferenceForId(next).get(
		ManagedObject.class);
	    Class<?> objectClass = object.getClass();
	    if (!"com.sun.sgs.impl.service.task.PendingTask".equals(
		    objectClass.getName()))
	    {
		count++;
	    }
