...
final class ManagedReferenceImpl implements ManagedReference, Serializable {
	...
	Context context, ManagedObject object, boolean newArea)
    {
	assert object != null : "Object is null";
	ManagedReferenceImpl ref = context.refs.find(object);
	if (ref == null) {
	    ref = new ManagedReferenceImpl(context, object, newArea);
	...
	oid = context.store.createObjectNear(
	    context.txn,
	    newArea ? DataStore.NEAR_NEW_REGION : DataStore.NEAR_UNSPECIFIED);
	this.object = object;
	state = State.NEW;
	validate();
    }
