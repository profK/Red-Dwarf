    ...
    implements DataService, TransactionParticipant, ProfileProducer
{
	...
	try {
	    ...
	    Context context = getContext();
	    ManagedReference result = context.getReference(object, false);
    ...
    public void manageInNewArea(ManagedObject object) {
	try {
	    if (object == null) {
		throw new NullPointerException("The object must not be null");
	    } else if (!(object instanceof Serializable)) {
	    ...
	    Context context = getContext();
	    context.getReference(object, true);
	    logger.log(
		Level.FINEST, "manageInNewArea object:{0} returns", object);
	} catch (RuntimeException e) {
	    logger.logThrow(
		Level.FINEST, e, "manageInNewArea object:{0} throws", object);
	    throw e;
	}
    }
